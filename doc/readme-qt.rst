FazzCoin-qt: Qt4 GUI for FazzCoin
===============================
...
Alternatively, install Qt Creator and open the `FazzCoin-qt.pro` file.

An executable named `FazzCoin-qt` will be built.
...
A warning for people using the *static binary* version of FazzCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of FazzCoin is linked against libdb5.3.
...
installed causes FazzCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
