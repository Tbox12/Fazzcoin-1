...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #FazzCoinTEST2\r");
            Send(hSocket, "WHO #FazzCoinTEST2\r");
        } else {
            // randomly join #FazzCoin00-#FazzCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #FazzCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #FazzCoin%02d\r", channel_number).c_str());
        }
